...
function ErrorToken({ token }) {
  const { acceptCorrection, rejectCorrection, editCorrection, selectedTokenId, selectToken } = useAnalysis();
  ...
  const getPatternColor = () => {
    ...
    if (pattern.includes('phonetic') || pattern.includes('dental') || pattern.includes('retroflex')) {
  ...
  const getTokenClasses = () => {
    const baseClasses = 'inline-block cursor-pointer rounded px-1 py-0.5 transition-all relative';
  ...
  return (
    <span className="relative inline">
      ...
      <span
        ...
        title={token.state === WORD_STATES.FLAGGED ? 'Click to review' : undefined}
      >
        {token.displayWord}
        ...
        >
          ...
          <div className="px-4 py-3 bg-gradient-to-r from-red-50 to-orange-50 border-b border-red-100">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-red-800">Detected Error</span>
          ...
          <div className="p-4">
            ...
            <div className="mb-4">
              ...
              <p className="text-sm font-medium text-slate-700">{token.pattern}</p>
            </div>
